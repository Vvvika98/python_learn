def square(number):        
    ...
    if 0 < number < 65:
        return 2 ** (number-1)   #кол-во зерен на каждой клетке удваивается, начиная с 1 на первой клетке (2 в степени 0). В общем, вычитание 1 преобразует номер клетки в правильный показатель степени двойки для вычисления количества зерен.
    raise ValueError("square must be between 1 and 64")  #если значение клетки вне допустимого диапазона        
